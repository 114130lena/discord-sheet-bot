...
def _can_use_session(project, interaction, allow_admin=True):
    return can_access(project, guild_id=interaction.guild_id, channel_id=interaction.channel_id, user_id=interaction.user.id, allow_admin=allow_admin and _is_admin(interaction))
...
async def _start_session_batch(channel, status_message, project):
    ...
    if old_delete:
        old_delete.cancel()
    state = {"status_message": status_message, "images": [], "timer_task": None, "debounce_task": None, "project": project}
...
run_analysis_replacement = r'''async def run_analysis(channel, state):
    ...
    try:
        ...
        if project is None:
            project = new_session(guild_id=channel.guild.id if channel.guild else None, channel_id=channel.id, owner_id=None)
        merge = add_batch(project, result.get("teams", []), len(images))
        if not merge.get("ok"):
            await status.edit(content=f"❌ **세션은 최대 {MAX_TEAMS}팀까지 등록할 수 있습니다.**\n현재 {len(project.get('teams', []))}팀이 등록되어 있어 이번 분석 결과는 반영하지 않았습니다.", embed=project_embed(project), view=ProjectView(project, save_project_to_sheet, apply_db_updates))
        ...
        message = (f"📋 **세션 분석 완료.**\n세션: **{_session_label(project)}**\n"
                   ...
                   f"필요하면 `/세션추가 {project.get('session_id')}`로 이 세션을 이어서 작업할 수 있습니다.")
        ...
        result_delete_tasks[channel.id] = asyncio.create_task(delete_result_later(status, 300))
        print(f"Gemini 분석 완료: 세션 {project['session_id']} / 묶음 {len(batches)} / 누적 {total}/{MAX_TEAMS}팀 / 이미지 {len(images)}장")
...
async def start_analysis(interaction, 대회명: str = "", 세션명: str = ""):
    ...
    if channel_id in analysis_waiting:
        ...
        return
    project = new_session(event_name=대회명, session_name=세션명, guild_id=interaction.guild_id, channel_id=interaction.channel_id, owner_id=interaction.user.id)
    current_projects[channel_id] = project
    label = _session_label(project)
...
async def session_list_command(interaction):
    ...
    for p in visible[:20]:
        ...
        status = {"active":"진행 중", "paused":"보류", "completed":"완료"}.get(p.get("session_status"), "진행 중")
        lines.append(f"• **{_session_label(p)}** · `{sid}` · {p.get('event_name') or '대회 미지정'} · {len(p.get('teams', []))}/{MAX_TEAMS}팀 · {status}")
...
async def session_info_command(interaction, 세션ID: str):
    ...
    if not project or not _can_use_session(project, interaction):
        ...
        return
    batches = project.get("session_batches", [])
    text = (f"📂 **{_session_label(project)}**\n세션 ID: `{project.get('session_id', project.get('id'))}`\n"
            f"대회: **{project.get('event_name') or '-'}**\n상태: **{project.get('session_status', 'active')}**\n"
            f"팀: **{len(project.get('teams', []))}/{MAX_TEAMS}**\n생성: `{project.get('session_created_at', '-')}`\n"
...
async def session_load_command(interaction, 세션ID: str):
    ...
    mark_status(project, "active")
    await interaction.response.send_message(f"📂 **세션을 불러왔습니다.**\n**{_session_label(project)}** · 현재 **{len(project.get('teams', []))}/{MAX_TEAMS}팀**", embed=project_embed(project), view=ProjectView(project, save_project_to_sheet, apply_db_updates), ephemeral=True)
...
async def session_add_command(interaction, 세션ID: str):
    if interaction.channel_id not in analysis_channels:
        await interaction.response.send_message("❌ 이 채널은 전력분석 채널이 아닙니다.", ephemeral=True); return
    ...
    project = get_session(세션ID)
    if not project or not _can_use_session(project, interaction):
        await interaction.response.send_message("❌ 해당 세션을 찾을 수 없거나 접근 권한이 없습니다.", ephemeral=True); return
    if len(project.get("teams", [])) >= MAX_TEAMS:
        await interaction.response.send_message(f"❌ 이 세션은 이미 {MAX_TEAMS}팀으로 가득 찼습니다.", ephemeral=True); return
    current_projects[interaction.channel_id] = project
    await interaction.response.send_message(f"➕ **{_session_label(project)}**에 추가 분석을 시작합니다.\n현재 **{len(project.get('teams', []))}/{MAX_TEAMS}팀**\n사진을 업로드해 주세요.\n⏱️ **30초 후 자동 분석됩니다.**")
    status = await interaction.original_response()
    await _start_session_batch(interaction.channel, status, project)
...
async def session_pause_command(interaction, 세션ID: str):
    ...
    await interaction.response.send_message(f"⏸️ **{_session_label(project)}** 세션을 보류했습니다.", ephemeral=True)
...
async def session_complete_command(interaction, 세션ID: str):
    ...
    if current_projects.get(interaction.channel_id, {}).get("id") == project.get("id"):
        current_projects.pop(interaction.channel_id, None)
    await interaction.response.send_message(f"✅ **{_session_label(project)}** 세션을 완료했습니다.", ephemeral=True)
...
if 'from session_manager import' not in source:
    raise RuntimeError("session manager import injection failed")
