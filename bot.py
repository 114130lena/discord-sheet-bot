...
def apply_db_updates(project):
    ...
    added_teams = 0; added_players = 0; transfers = 0
    for team in suggestions.get("teams", []):
        if add_team(team.get("name", ""), tag=team.get("tag", "")): added_teams += 1
    for player in suggestions.get("players", []):
        if add_player(player.get("name", ""), team=player.get("team", "")): added_players += 1
    for change in suggestions.get("transfers", []):
        result = update_player_team(change.get("name", ""), change.get("team", ""))
        if result.get("status") in {"changed", "set", "new"}: transfers += 1
...
async def debounce_analysis(channel, state):
    try:
        await asyncio.sleep(2); await run_analysis(channel, state)
    except asyncio.CancelledError: return
...
@bot.event
async def on_ready():
    print("=" * 50); print(f"로그인 완료: {bot.user}"); print(f"서버 수: {len(bot.guilds)} / 분석 채널: {len(analysis_channels)}개"); print("=" * 50)
    try:
    ...
    except Exception as e: print(f"Slash Command 동기화 오류: {type(e).__name__}: {e}")
...
async def reset_analysis(interaction):
    channel_id = interaction.channel_id; state = analysis_waiting.pop(channel_id, None)
    if state:
        for key in ("timer_task", "debounce_task"):
            if state.get(key): state[key].cancel()
...
async def player_add(interaction, 선수명: str, 팀: str = "", 메모: str = ""):
    add_player(선수명, team=팀, notes=메모); await interaction.response.send_message(f"✅ 선수 DB에 **{선수명}** 등록 완료.", ephemeral=True)
...
async def team_add(interaction, 팀명: str, 약칭: str = "", 메모: str = ""):
    add_team(팀명, tag=약칭, notes=메모); await interaction.response.send_message(f"✅ 팀 DB에 **{팀명} [{약칭}]** 등록 완료.", ephemeral=True)
